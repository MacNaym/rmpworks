      ...
      <section className={`${frame} py-20 sm:py-28 lg:py-32`}>
        <div className="grid gap-12 md:grid-cols-12">
          ...
          <div className="md:col-span-9">
            <h2 className="max-w-6xl text-4xl font-semibold leading-[0.98] tracking-[-0.055em] sm:text-6xl lg:text-8xl">Small teams.<br />Fast decisions.<br /><span className="text-[var(--rmp-green)]">Useful output.</span></h2>
            <div className="mt-14 grid gap-8 border-t border-[var(--line)] pt-8 sm:grid-cols-3">
              <div><p className="font-mono text-xs text-[var(--rmp-purple)]">01 / FOCUS</p><p className="mt-4 max-w-sm leading-7">Solve one valuable problem before adding ten features nobody asked for.</p></div>
              <div><p className="font-mono text-xs text-[var(--rmp-green)]">02 / SHIP</p><p className="mt-4 max-w-sm leading-7">Put working products in front of real people early and improve from evidence.</p></div>
              <div><p className="font-mono text-xs text-[var(--rmp-orange)]">03 / SYSTEMIZE</p><p className="mt-4 max-w-sm leading-7">Automate repeatable work so the product can grow without growing bureaucracy with it.</p></div>
            </div>
          </div>
        </div>
      </section>
