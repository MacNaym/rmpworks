  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[var(--background)] text-[var(--foreground)]">
      <header className="border-b border-[var(--line)]">
        <div className={`${frame} flex items-center justify-between py-6`}>
          <Link href="/" className="flex items-center gap-3 text-base font-extrabold tracking-[-0.035em]">
            <span className="h-2.5 w-2.5 rounded-full bg-[var(--accent)]" aria-hidden />
            RMP WORKS
          </Link>

          <div className="flex items-center gap-5 sm:gap-8">
            <nav className="hidden items-center gap-6 text-[0.92rem] text-[var(--muted)] sm:flex sm:gap-8">
      ...
      <section className={`${frame} grid min-h-[74vh] content-between py-8 sm:py-12 lg:py-14`}>
        ...
        <div className="py-14 sm:py-20 lg:py-24">
          <h1 className="max-w-[1500px] text-[clamp(4rem,8.5vw,9.5rem)] font-semibold leading-[0.84] tracking-[-0.075em]">
            Costruiamo cose
            <br />
        ...
        <div className="grid gap-8 border-t border-[var(--line)] pt-6 md:grid-cols-12">
          ...
          <div className="flex items-end md:col-span-3 md:col-start-10 md:justify-end">
            <Link href="#work" className="inline-flex items-center gap-3 text-sm font-semibold">
      ...
      <section id="work" className="border-y border-[var(--line)] bg-[#101010] text-[#f4f1ea]">
        <div className={`${frame} py-16 sm:py-20 lg:py-24`}>
          <div className="grid gap-8 border-b border-white/20 pb-10 md:grid-cols-12">
            ...
            <h2 className="max-w-6xl text-4xl font-semibold tracking-[-0.045em] sm:text-5xl md:col-span-9 lg:text-7xl">
          ...
          <div>
            {ventures.map((venture) => (
              <Link href={venture.href} key={venture.name} className="group grid gap-6 border-b border-white/20 py-10 transition hover:border-white/60 md:grid-cols-12 md:items-start">
                ...
                <p className="max-w-3xl text-[1.02rem] leading-7 text-white/72 md:col-span-5">{venture.description}</p>
        ...
        <div className="grid gap-12 md:grid-cols-12">
          ...
          <div className="md:col-span-9">
            ...
            <div className="mt-14 border-t border-[var(--line)]">
              <div className="grid gap-5 border-b border-[var(--line)] py-7 md:grid-cols-12 md:items-start">
                <p className="font-mono text-xs text-[var(--muted)] md:col-span-2">01 / FOCUS</p>
                <p className="max-w-3xl text-lg leading-7 md:col-span-8">Risolviamo un problema che conta prima di aggiungere dieci funzioni che nessuno ha chiesto.</p>
              </div>
              <div className="grid gap-5 border-b border-[var(--line)] py-7 md:grid-cols-12 md:items-start">
                <p className="font-mono text-xs text-[var(--muted)] md:col-span-2">02 / LANCIA</p>
                <p className="max-w-3xl text-lg leading-7 md:col-span-8">Mettiamo presto prodotti funzionanti davanti a persone reali e miglioriamo usando evidenze.</p>
              </div>
              <div className="grid gap-5 border-b border-[var(--line)] py-7 md:grid-cols-12 md:items-start">
                <p className="font-mono text-xs text-[var(--muted)] md:col-span-2">03 / SISTEMA</p>
                <p className="max-w-3xl text-lg leading-7 md:col-span-8">Automatizziamo il lavoro ripetibile così il prodotto può crescere senza far crescere la burocrazia.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="bg-[var(--accent)] text-white">
        <div className={`${frame} grid min-h-[54vh] content-between py-10 sm:py-14`}>
          ...
          <div className="flex flex-col gap-6 border-t border-white/40 pt-7 sm:flex-row sm:items-center sm:justify-between">
            <p className="max-w-xl text-lg text-white/85">Prodotti, esperimenti e business con una ragione per esistere.</p>
            <Link href="/contact" className="inline-flex w-fit items-center rounded-full bg-[#101010] px-7 py-4 text-base font-bold text-white transition hover:-translate-y-0.5 hover:bg-white hover:text-[#101010]">
              Parliamone →
            </Link>
          </div>
        </div>
      ...
      <footer className="border-t border-white/10 bg-[#101010] text-white">
        <div className={`${frame} flex flex-col gap-10 py-14 md:flex-row md:items-end md:justify-between`}>
          <div>
            <p className="text-xl font-bold tracking-[-0.035em]">RMP WORKS</p>
            <p className="mt-3 max-w-md text-sm leading-6 text-white/45">Studio indipendente di prodotti, software e piccoli business costruiti per risolvere problemi reali.</p>
            <p className="mt-5 text-sm text-white/35">© 2026 RMP Works</p>
          </div>
